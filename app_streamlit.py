...
def recognize_face(image):
    """Retourne (nom, distance, visage détecté) pour une image PIL ou ndarray."""
    ...
    if uploaded_file is not None:
        ...
        if st.button(" Analyser l'image"):
            ...
            if name is None:
                ...
                if name == "Inconnu":
                    st.markdown(f'<div class="result-card warning-card"> Visage inconnu (distance = {distance:.3f})</div>', unsafe_allow_html=True)
                else:
...
# ---------- WEBCAM STREAMLIT ----------
    ...
    FRAME_WINDOW = st.image([])  # Placeholder pour flux webcam
